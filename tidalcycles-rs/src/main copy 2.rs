...
"#;
    ...
    let mut sc: tokio::process::Child = TokioCommand::new(&sclang)
        .arg("-D")
        .arg("startup.scd")
        // .stdout(std::process::Stdio::piped())
        // .stderr(std::process::Stdio::piped())
        .spawn()?;
    children.lock().unwrap().push(sc);
    // Take ownership of the child process to call wait_with_output
    let mut sc = children.lock().unwrap().pop().unwrap();
    ...
    if sc_output.contains("SuperDirt installed. Please restart SuperCollider.") {
        ...
        let mut sc2 = TokioCommand::new(&sclang)
            ...
            .stderr(std::process::Stdio::piped())
            .spawn()?;
        children.lock().unwrap().push(sc2);
        // Take ownership of the child process to call wait_with_output
        let mut sc2 = children.lock().unwrap().pop().unwrap();
