...
use super::{Event, IpcError, IpcResult, AppId, IpcPayload};
...
impl EventSubscriber {    /// Create a new event subscriber for events from the specified app
    ...
    pub fn try_receive(&mut self) -> IpcResult<Vec<Event>> {
        ...
        while let Some(sample) = self.subscriber
            .receive()
            .map_err(|_| IpcError::ReceiveError(format!("Failed to receive")))? 
        {
            ...
            if let Ok(event) = serde_json::from_slice::<Event>(json_data) {
                ...
                return Err(IpcError::DeserializationError("Failed to deserialize event data".to_string()));
            }
        }
